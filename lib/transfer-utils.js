...
// 16 bytes, base64url, no padding. This is the room capability, not a PIN.
...
/** Accept a room id or a full share URL. Anything else is not a capability. */
function parseRoomInput(raw) {
  if (typeof raw !== "string") return null;
  const trimmed = raw.trim();
  if (validateRoomId(trimmed)) return trimmed;
  try {
    const peer = new URL(trimmed).searchParams.get("peer");
    if (validateRoomId(peer)) return peer;
  } catch {
    /* not a URL */
  }
  return null;
